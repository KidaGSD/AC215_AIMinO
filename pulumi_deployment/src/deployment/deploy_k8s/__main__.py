...
# Setup Containers (AIMinO API only)
api_service = setup_containers(project, namespace, k8s_provider, ksa_name, app_name)

# Setup Load Balancer (routes directly to AIMinO API)
if setupSSL:
    ip_address, ingress, host = setup_loadbalancer_ssl(
        namespace, k8s_provider, api_service, None, app_name
    )
else:
