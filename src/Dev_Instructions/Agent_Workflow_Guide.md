...
|-------|-------------|-------|
| Napari Client | `aimino_frontend/napari_app/main.py` launcher, `aimino_core.executor`, command dock widget | Provides real-time viewer control using the shared command registry. |
...
| Shared Utilities | `aimino_frontend/aimino_core/command_models.py`, `aimino_core/executor.py` (installed) | Single source of truth for schemas and command execution logic. |

---

## 3. Required Setup Steps

1. Create and activate the Conda environment: `conda env create -f environment.yml && conda activate aimino` (for existing env run `conda env update -f environment.yml --prune`).  
2. Install the shared package: `pip install -e aimino_frontend/aimino_core`.  
